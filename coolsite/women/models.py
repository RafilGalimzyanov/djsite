...
class Women(models.Model): #Наследуем от базового класса model, поле id прописано в Model
    ...
    content = models.TextField(blank=True) # Текст статьи, параметрр значит, что поле может быть пустым
    photo = models.ImageField(upload_to='photos/%Y/%n/%d/') # Ссылкаа на фотографию поста
